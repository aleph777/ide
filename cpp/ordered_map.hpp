/*=============================================================================
  Copyright © 2023 - 2023 -  Thomas Fontaine.  All Rights Reserved.
...
public:
   ...
   * @return:   boolean flag
   */
  inline bool exists(Key key) const { return m_.contains(key); }
   ...
   * @return:   list of keys
   */
  inline const ListKey keys() const { return keys_; }

  /*
   * @brief:    get the keys of the map
   * @return:   vector of keys
   */
  inline const VectorKey keys_v() const { return VectorKey(keys_.begin(), keys_.end()); }
  ...
  bool operator!=(const OrderedMap<Key, Value>& other) { return !(operator==(other)); }

  Value& operator[](Key key) { return m_[key]; };

  friend std::ostream& operator<<(std::ostream& os, const OrderedMap<Key, Value>& m)
  {
    os << "{\n";
