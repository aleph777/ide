...
;; Time-stamp:	<99/08/14 19:24:35 vinicius>
;; Version: 1.2
;; Keywords: internal
...
(defun pretty-rectangle-max (startpos &optional ignore ignore)
  ...
  (let ((ncol 1)
    ...
    (setq values (cons (- (current-column) origin)
		       values))
    (if pcol-max
	;; extend pcol-max, if needed
	(let ((index (length pcol-max)))
	  (and (> ncol index)
	       (let ((extend (make-vector ncol 0)))
		 (while (> index 0)
		   (setq index (1- index))
		   (aset extend index (aref pcol-max index)))
		 (setq pcol-max extend))))
      ;; initialize pcol-max
      (setq pcol-max (make-vector ncol 0)))
    ;; get maximum column length
    (while values
      (setq ncol (1- ncol))
      (aset pcol-max ncol (max (aref pcol-max ncol) (car values)))
